%PIRK4 is an implementation of Projective Integration with the
%macrosolver given by an explicit fourth order Runge--Kutta scheme and a user-specified microsolver. JM,
%July 2018.
%!TEX root = ../equationFreeDoc.tex
%{
\subsection{\texttt{PIRK4()}}
\label{sec:PIRK4}
...
\begin{matlab}
%}
function varargout = PIRK4(micro,tspan,IC)
%{
\end{matlab}
...
\verb|t_in|, the initialisation time, \verb|x_in| \(\in\RR^n\),
 the initial state, and \verb|bT|, the total time of simulation. \\
 Outputs:
 \verb|tout|, the vector of solution times, and \verb|xout|, the corresponding states.
  \end{itemize}
  \end{itemize}
  The remaining inputs to \verb|PIRK4()| relate to the macroscale solution.
  \begin{itemize}
...
\paragraph{Output}
Standard usage is to output only estimates of the state at the times given in \verb|tspan|, with the following usage:\\

\verb|x = PIRK4(micro,tspan,IC)|
\begin{itemize}
...
\begin{itemize}
\item  \verb|xm|, a cell array containing the output of the applications of the microsolver that immediately follow a PI macrostep. \verb|xm{1}| is an \(L\) dimensional column vector containing times; \verb|xm{2}| is an \(L\times n\) array of the corresponding microsolver output. The data in \verb|xm| is an accurate simulation and can be used alongside \verb|x| when visualising the solution. \end{itemize}
\verb|[x,xm,xr] = PIRK4(micro,tspan,IC)|
\begin{itemize}
\item \verb|xr|, a cell array containing the `remaining' applications of the microsolver required by the PI method during the calculation of the macrostep. The applications of the microsolver in \verb|xr| do not have the same physical interpretation as those in \verb|xm|; they are required in order to estimate the slow vector field during the calculation of the Runge--Kutta increments, and will not in general have good error statistics.
\end{itemize}
\verb|[x,xm,xr,dx] = PIRK4(micro,tspan,IC)|
\begin{itemize}
...
\end{matlab}
    ...
    Run the first application of the microsolver; since this application directly follows from the initial conditions, or from the latest macrostep, this microscale information is physically meaningful as a simulation of the system.\\
     Extract the length of the burst in time and the size of the final time step. 
   \begin{matlab}
%}
...
\end{matlab}
      ...
      Find the needed time step to reach time \verb|tspan(n+1)|, form a first estimate \verb|dx1| of the slow vector field.
\begin{matlab}
%}
    Dt=tspan(n+1)-t-bT;  
...
\end{matlab}
  ...
  Project along \verb|dx1| to form an intermediate approximation of \verb|x|; repeat the above steps three more times in order to form four approximations of the slow vector field.
\begin{matlab}
%}
...
\begin{matlab}
%}
    x(n+1,:) = xm1(end,:) + Dt*(dx1+2*dx2+2*dx3+dx4)/6;
%{
\end{matlab}
...
%}   
        if save_full_micro 
            t_r_micro{n+1} = [reshape(t2,[],1); reshape(t3,[],1); reshape(t4,[],1)];
            x_r_micro{n+1} = [xm2; xm3; xm4];
%{
\end{matlab}
...
%}            
            if save_dx 
                dx_t{n} = [t1(end); t2(end); t3(end); t4(end)];
                dx_hist{n} = [dx1; dx2; dx3; dx4];
            end
        end
    end
%{
