% spmdChanDisp simulates shear dispersion alog a channel 
% with 1D patches as a Proof of Principle example of
% parallel computing with spmd.    AJR, Sep--Oct 2020
%!TEX root = ../Doc/eqnFreeDevMan.tex
%{
\section{\texttt{spmdChanDisp}: simulation of a 1D shear dispersion via simulation on
small patches}
\label{sec:spmdChanDisp}

Simulate shear dispersion along 1D space on 1D
patches as a Proof of Principle example of parallel
computing with \verb|spmd|.  


Choose one of four cases:
\begin{itemize}
\item \verb|theCase=1| is corresponding code without parallelisation (in this toy problem it is much the quickest because there is no expensive communication);
\item \verb|theCase=2| is for users happier to explicitly
invoke \verb|spmd|-blocks.
\item \verb|theCase=3| invokes projective integration for
...
y = linspace(-1,1,ny+1);
y=y(2:end)-diff(y)/2
cHetr = 3/2*(1-y.^2).*exp(0*randn([mPeriod 1 3]));
%{
\end{matlab}
...
\begin{matlab}
%}
if theCase==1, global patches, end
nPatch=9
nSubP=2*mPeriod+1
ratio=0.2
xLim=[0 nPatch/ratio] 
...
\end{matlab}
\begin{figure}
\centering \caption{\label{fig:spmdChanDispt0}initial
...
variables~\(y,z\).  \cref{fig:spmdChanDisptFin}
plots the nearly smooth field values at time \(t=0.4\). }
\includegraphics[scale=0.8]{spmdChanDispt0}
\end{figure}
...
Animate the solution field over time. Since the
