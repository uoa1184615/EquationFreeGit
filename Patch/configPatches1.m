...
%{
\end{matlab}
The code here shows one way to get started: a user's 
script may have the following three steps (arrows 
indicate function recursion).
...
ratio~\(0.2\), and with seven points within each patch.
\begin{matlab}
%}
configPatches1(@BurgersPDE,[0 2*pi], nan, 8, 0, 0.2, 7);
%{
\end{matlab}
Set an initial condition, and integrate in time using
standard functions.
\begin{matlab}
%}
...
leave the gaps.  \autoref{fig:ps1BurgersCtsU} illustrates an
...
(u_{i+1,j}-2u_{i,j}+u_{i-1,j}) -30u_{ij} \frac1{2\delta x}
(u_{i+1,j}-u_{i-1,j})\).
\begin{matlab}
%}
