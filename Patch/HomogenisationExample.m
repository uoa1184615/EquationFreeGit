...
in the core is an even multiple of the microscale periodicity.
We present two different methods of obtaining a macroscale solution.
One method uses the given heterogeneous diffusion, which produces a
solution which has microscale roughness (\cref{fig:ps1HomogenisationCtsU}). The other method constructs an
ensemble of heterogeneous diffusion and produces an ensemble average 
solution which has a smooth microscale (\cref{fig:ps1HomogenisationCtsUEnsAve}). 
...
The heterogeneous diffusion results in a smilarly heterogeneous  field solution.}
\includegraphics[scale=0.85]{../Patch/ps1HomogenisationCtsU}
\end{figure}%
\begin{figure}
\centering \caption{\label{fig:ps1HomogenisationCtsUEnsAve}the
diffusing field~\(u(x,t)\) in the patch (gap-tooth) scheme
applied to microscale heterogeneous diffusion with an ensemble average.
The ensemble average smooths out the heterogeneous diffusion.}
\includegraphics[scale=0.85]{../Patch/ps1HomogenisationCtsUEnsAve}
\end{figure}%
...
Establish global data struct~\verb|patches| for heterogeneous diffusion
solved on \(2\pi\)-periodic domain, with nine patches, each
patch of half-size~\(0.2\).
A user can add information to~\verb|patches| in order to communicate to the 
time derivative function. 
Quadratic (fourth-order) interpolation  \(\verb|ordCC|=4\) provides 
values for the inter-patch coupling conditions.
The odd integer \(\verb|patches.nCore|=3\) defines the size of the patch 
core (this must be larger than zero and less than \verb|nSubP|), where 
a core of size zero indicates that the value in the centre of the patch 
gives the macroscale. 
The introduction of a finite width core requires a redefinition of the 
half-patch ratio, as described by \cite{Bunder2013b}.
The Boolean \verb|patches.Ens| indicates whether or
not we apply ensemble averaging of diffusivity configurations.
...
the diffusivity coefficients within each patch.  
In the case of ensemble averaging, \verb|nVars| becomes the size of the 
ensemble (for the case of no ensemble averaging \verb|nVars| is the number of different 
field variables, which in this example is \(\verb|nVars|=1\)) and we use the ensemble described by \cite{Bunder2013b} which 
includes all reflected and translated configurations of \verb|patches.cDiff|.
With ensemble averaging we must increase the size of the diffusivity matrix 
to \((\verb|nSubP-1)|\times \verb|nPatch|\times \verb|nVars|\).
\begin{matlab}
%}
...
Plot the simulation in \cref{fig:ps1HomogenisationCtsU} (with no ensemble 
average) or \cref{fig:ps1HomogenisationCtsUEnsAve} (with an ensemble average).
If we have calculated an ensemble of field solutions, then we must first take
...
if patches.EnsAve
  print('-depsc2','ps1HomogenisationCtsUEnsAve')
else
...
heterogeneous diffusion with no ensemble average: different colours correspond to 
the times in the legend.
This field solution displays some fine scale heterogeneity due to the heterogeneous diffusion.}
\includegraphics[scale=0.85]{../Patch/ps1HomogenisationU}
\end{figure}%
\begin{figure}
\centering \caption{\label{fig:ps1HomogenisationUEnsAve}field
\(u(x,t)\) shows basic projective integration of patches of
heterogeneous diffusion with ensemble average: different colours correspond to 
the times in the legend.
Once transients have decayed, this field solution is smooth due to the ensemble average.}
\includegraphics[scale=0.85]{../Patch/ps1HomogenisationUEnsAve}
\end{figure}%
...
illustrated by \Cref{fig:ps1HomogenisationU,fig:ps1HomogenisationUEnsAve}.

This second part of the script implements the following design, where the 
micro-integrator could be, for example, \verb|ode45| or \verb|rk2int|.
\begin{enumerate} \def\itemsep{-1.5ex}
...
\cref{fig:ps1HomogenisationU} or \cref{fig:ps1HomogenisationUEnsAve}.
If we have calculated an ensemble of field solutions, then we must first take
the ensemble average. 
\begin{matlab}
%}
...
set(gcf,'PaperPosition',[0 0 14 10]);
if patches.EnsAve
  print('-depsc2','ps1HomogenisationUEnsAve')
else
...
in \cref{fig:ps1HomogenisationMicro} or \cref{fig:ps1HomogenisationMicroEnsAve}.
\begin{figure}
...
\end{figure}
\begin{figure}
\centering \caption{\label{fig:ps1HomogenisationMicroEnsAve}stereo
pair of the field~\(u(x,t)\) during each of the microscale
...
if patches.EnsAve
  print('-depsc2','ps1HomogenisationMicroEnsAve')
else
