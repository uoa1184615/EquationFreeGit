...
Quadratic (fourth-order) interpolation  \(\verb|patches.ordCC|=4\) provides 
...
patches.EnsAve=1;
configPatches1(@heteroDiff,[0 Len],nan,nPatch,patches.ordCC,patches.ratio,nSubP);
%{
\end{matlab}
...
field variables) and we use the ensemble described by \cite{Bunder2013b} which 
...
The heterogeneous diffusion is apparent in the field solution.}
...
the times in the legend.
This field solution is smooth due to the ensemble average.}
...
illustrated by \cref{fig:ps1HomogenisationU} and  \cref{fig:ps1HomogenisationUEnsAve}.
...
\item \verb|ode45| is similar to \item \verb|ode23|, but with reduced noise;
