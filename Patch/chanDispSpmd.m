% chanDispSpmd simulates 2D shear dispersion in a long
...
In this shear dispersion, although the micro-scale
diffusivities are one-ish, the shear causes an effective
longitudinal `diffusivity' of the order
of~\(\Pe^2\)\,---which is typically much larger than the
micro-scale diffusivity \cite[e.g.]{Taylor53}.
...
cross-channel diffusivity  \(\kappa(y)=(1-y^2)\) with noise.
...
\(x\)-spacing~\(\delta x\), and \(n_y\)~points over
\(|y|<1\) with spacing \(\delta y:=2/n_y\) at
\(y_j:=-1+(j-\tfrac12)\delta y\)\,, \(j=1:n_y\)\,.  A
microscale discretisation of \pde~\eqref{eq:pdeChanDisp} is
then
\begin{align}&
...
patches = configPatches1(@chanDispMicro, [0 Len], nan ...
    , nPatch, 0, ratio, nSubP, 'EdgyInt',true  ...
