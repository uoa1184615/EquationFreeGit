...
\begin{matlab}
%}
function u=patchCoreEdgeInt1(u)
global patches
%{
\end{matlab}
...
So compute centred differences of the ensemble averaged patch core values 
for the macro-interpolation of all fields. Assumes the domain
is macro-periodic.
\begin{matlab}
%}
...
if patches.ordCC>0 % then non-spectral interpolation
  ...
  if patches.alt % use only odd numbered neighbours
    dmu(1,:,:)=r*(ucore(jp,:)+ucore(jm,:))/2; % \mu
    dmu(2,:,:)=r^2*(ucore(jp,:)-ucore(jm,:)); % \delta
    jp=jp(jp); jm=jm(jm); % increase shifts to \pm2
  else % standard
    dmu(1,j,:)=r*(ucore(jp,:)-ucore(jm,:))/2; % \mu\delta
    dmu(2,j,:)=r^2*(ucore(jp,:)-2*ucore(j,:)+ucore(jm,:))/2; % \delta^2
  end% if odd/even
%{
\end{matlab}
...
\begin{matlab}
%}
  for k=1:(patches.ordCC/2-1) 
    dmu(2*k+1,:,:)=(r^2-k^2)*(dmu(2*k-1,jp,:)-2*dmu(2*k-1,j,:)+dmu(2*k-1,jm,:))/((2*k)*(2*k+1));
    dmu(2*k+2,:,:)=(r^2-k^2)*(dmu(2*k,jp,:)-2*dmu(2*k,j,:)+dmu(2*k,jm,:))/((2*k+2)*(2*k+1));
  end
%{
\end{matlab}
