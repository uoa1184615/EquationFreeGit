...
\cref{fig:ps1HomogenisationCoreCtsU} shows an example simulation
...
rng('default'); rng(1);
cDiff = exp(4*rand(mPeriod,1)); %2*abs(randn(mPeriod,1)); %exp(randn(mPeriod,1)); %2*abs(randn(mPeriod,1)); % 
...
this must be increase to \(\verb|nSubP-1|\times \verb|nPatch|\times nVars\) 
...
(this must be larger than zero and less than \verb|nSubP|). 
...
Plot the simulation in \cref{fig:ps1HomogenisationCoreCtsU}.
...
illustrated by \cref{fig:ps1HomogenisationCoreU}.
...
\cref{fig:ps1HomogenisationCoreU}.
If we have calculated an ensemble of field solutions, we must first take
the ensemble average. 
\begin{matlab}
%}
...
in \cref{fig:ps1HomogenisationCoreMicro}.
\begin{figure}
