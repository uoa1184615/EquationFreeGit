...
of the computation: patches are equi-spaced over the
interior of the rectangle~\([\verb|Xlim(1)|,\verb|Xlim(2)|]
\times [\verb|Xlim (3)|,\verb|Xlim(4)|]\): if of length two,
then use the same interval in both directions, otherwise
\verb|Xlim(1:4)| give the interval in each direction.
...
title('Nonlinear diffusion PDE on patches: solving with initial condition')
...
for i=1:length(ts)
  ...
  hsurf.ZData = u';
  title(['Nonlinear diffusion PDE on patches: time = ' num2str(ts(i))])
  pause(0.1)
end
%{
\end{matlab}
...
% should avoid this next implicit auo-replication
