...
The following charts confuse me??  and we must \emph{not} resize\slash scale fixed width constructs.  Use linewidth for large-scale layout scaling, em for small-widths, and ex for small-heights. }
\newlength{\WD}%% to set overall size for each chart, propto linewidth
\begin{figure}
\caption{\label{fig:constructPI}The Projective Integration (PI) method greatly accelerates simulation\slash integration of a system exhibiting multiple time scales.
The PI \cref{sec:ProjInt} presents several `main' functions that could separately be invoked to perform PI, as well as several optional wrapper functions that may be invoked. 
This chart overviews constructing a PI simulation, whereas \cref{fig:PIchoosemacro} roughly guides which top-level PI functions should be used.
...
\begin{figure}
\centering
\caption{\label{fig:PIchoosemacro}The Projective Integration (PI) method greatly accelerates simulation\slash integration of a system exhibiting multiple time scales.
In conjunction with \cref{fig:constructPI}, this chart roughly guides which top-level PI functions should be used.
\cref{sec:ProjInt} fully details each function.}
