...
If required patch interpolations are not defined, then assume that all
patches are interpolated.
If no boundary conditions are specified, then assume no boundary conditions
are required. 
\begin{matlab}
%}
if isempty(patches.intX)  
    intX = 1;
else
    intX = patches.intX;
end
if isempty(patches.intY)
    intY = 1;
else
    intY = patches.intY;
end
if isempty('patches.BC')
    patches.BC =  @(uu, patches) uu;
end
...
\begin{matlab}
%}
% uinterp = patchEdgeIntCore(1,uinterp,x,patches,stagShift ...
%     ,1,nx,(ny-2)*nVars,nEnsem,1,Nx,Ny,patches.le,patches.ri);
% uinterp = patchEdgeIntCore(2,uinterp,y,patches,stagShift ...
%     ,nx,ny,      nVars,nEnsem,Nx,Ny,1,patches.bo,patches.to);
% uinterp = reshape(uinterp,nx,ny,nVars,nEnsem,Nx,Ny);

% in staggered grids, we might want corners! So let's always do corners too
uinterp = patchEdgeIntCore(1,uinterp,x,patches,stagShift ...
    ,1,nx,(ny-2)*nVars,nEnsem,1,Nx,Ny,patches.le,patches.ri);
uinterp = patchEdgeIntCore(2,uinterp,y,patches,stagShift ...
    ,nx,ny,      nVars,nEnsem,Nx,Ny,1,patches.bo,patches.to);
...
%{
\end{matlab}
Apply either interpolation or boundary conditions to patch edges.
\begin{matlab}
%}

u = patches.BC(uinterp,patches);
% u([1 end],:,:,:,:,:) = ...
%     reshape(intX,[1 1 1 1 size(intX)]).*uinterp([1 end],:,:,:,:,:) ...
%    + reshape(1-intX,[1 1 1 1 size(intX)]).*u([2 end-1],:,:,:,:,:);
% u(:,[1 end],:,:,:,:) = ...
%     reshape(intY,[1 1 1 1 size(intY)]).*uinterp(:,[1 end],:,:,:,:) ...
%    + reshape(1-intY,[1 1 1 1 size(intY)]).*u(:,[2 end-1],:,:,:,:);
% u([1 end],[1 end],:,:,:,:) = NaN;
