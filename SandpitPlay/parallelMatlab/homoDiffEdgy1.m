...
nVars=mPeriod^2
cHetr=exp(0.3*randn(mPeriod,3*nVars));
...
nPatch = 9
ratio = 0.25;
nSubP = mPeriod+2 %randi([mPeriod+1 2*mPeriod+2])
nEnsem = mPeriod % number realisations in ensemble??
if mod(nSubP,mPeriod)==2, nEnsem=1, end
configPatches1(@heteroDiff31,[-pi pi],nan,nPatch ...
...
\begin{matlab}
%}
u0 = 0.8*exp(-patches.x.^2)+0.2*rand(nSubP,nVars,nEnsem,nPatch);
