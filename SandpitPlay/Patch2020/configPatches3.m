...
\verb|patchSmooth3()|. \cref{sec:configPatches3eg} lists an
example of its use.
\begin{matlab}
%}
...
naturally stiff, but \verb|ode23| is quicker.
\begin{matlab}
%}
...
for i = 1:length(ts)
  ...
  u = uv(:,:,:,1,:);
  for p=1:2, subplot(1,2,p)
    set(uscat(p),'CData',col(u(:)),'SizeData',pix(u(:)));
