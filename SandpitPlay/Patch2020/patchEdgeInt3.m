...
\begin{matlab}
%}
dx = patches.x(3,1,1,1,1,1,1,1)-patches.x(2,1,1,1,1,1,1,1);
DX = patches.x(2,1,1,1,1,2,1,1)-patches.x(2,1,1,1,1,1,1,1);
if patches.EdgyInt==0, rx = dx*(nx-1)/2/DX;
    else               rx = dx*(nx-2)/DX;
    end
dy = patches.y(1,3,1,1,1,1,1,1)-patches.y(1,2,1,1,1,1,1,1);
DY = patches.y(1,2,1,1,1,1,2,1)-patches.y(1,2,1,1,1,1,1,1);
if patches.EdgyInt==0, ry = dy*(ny-1)/2/DY;
    else               ry = dy*(ny-2)/DY;
    end
dz = patches.z(1,1,3,1,1,1,1,1)-patches.z(1,1,2,1,1,1,1,1);
DZ = patches.z(1,1,2,1,1,1,1,2)-patches.z(1,1,2,1,1,1,1,1);
if patches.EdgyInt==0, rz = dz*(nz-1)/2/DZ;
    else               rz = dz*(nz-2)/DZ;
    end
%{
\end{matlab}
...
if ordCC>0 % then finite-width polynomial interpolation
  ...
  if patches.stag % use only odd numbered neighbours
    ...
    dmuz(1,:,:,:,:,:,:,:,K) = (uCorez(:,:,:,:,:,:,:,Kp) -uCorez(:,:,:,:,:,:,:,Km))/2; % \mu\delta 
    dmuz(2,:,:,:,:,:,:,:,K) = (uCorez(:,:,:,:,:,:,:,Kp) -2*uCorez(:,:,:,:,:,:,:,K) +uCorez(:,:,:,:,:,:,:,Km)); % \delta^2
  end% if odd/even
%{
\end{matlab}
