...
% as an example application of patches in space.  From
% Leitenmaier & Runborg, http://arxiv.org/abs/2108.09463
% AJR, Sep 2021
...
field~\(\Mv(x,t)=(u,v,w)\) of the gap-tooth scheme applied to the
heterogeneous Landau--Lifshitz \pde\ to show the emergent macroscale
wave-like dynamics. This simulation uses eleven patches in space of size ratio~\(0.055\).  Compare the time \(t=0.1\) graph with Fig.~2.1 of \cite{Leitenmaier2021}.}
...
\cref{fig:homoLanLifU2} shows one patch simulation of this
...
\verb|ode15s| has no problems for \(\alpha=0.001\).  \footnote{But, add
randomness to the initial conditions and the computation
appears unstable with \texttt{ode15s} when \(\alpha<0.2\).
However, \texttt{ode23} may be stable? for \(\alpha=0.01\)
albeit expensively taking~\(10^7\) time-steps per second
(due to microscale oscillations of frequency up
to~\(10^5\)--\(10^6\)).}
\begin{matlab}
%}
global alpha ex5p1
...
with maybe 24~patches, but 11~is enough, here each patch of size ratio to fit
one period of the heterogeneity in each patch, and spectral
inter-patch interpolation to provide the patch edge-values. Invoking
\verb|EdgyInt| means the edge-values come from interpolating
the opposite next-to-edge values of the patches (not the
mid-patch values).  
\begin{matlab}
%}
global patches
...
%{
\end{matlab}
Reshape results for processing.  For simplicity, set edge values to \verb|nan|s. For the field values (which are rows
in~\verb|us|) we need to reshape, permute,  and reshape
again. 
\begin{matlab}
%}
...
%{
\end{matlab}
Check on constancy of~\(|\Mv(x,t)|\) in time.  The mean and standard deviation appears to show that, with~\verb|ode15s|, they are constant
to errors typically~\(10^{-5}\).
\begin{matlab}
%}
...
shown by \cref{fig:homoLanLif1DSpec}, with
\(N = \verb|nPatch|\) and \(n = \verb|nSubP| -2\)\,, and on base
of \(\Mv = \vec 1/\sqrt3\).
\begin{figure}
\centering\caption{\label{fig:homoLanLif1DSpec}spectrum of eigenvalues of the multiscale patch scheme applied to the Landau--Lifshitz \pde.  The macroscale eigenvalues are clearly separated from those of the microscale sub-patch modes.}
...
in time, for every~\(x\) (possibly requiring some filtering
to ensure simulations are not polluted by microscale
noise?). Presumably near zero (roughly~\(10^{-2}\)) due to
round-off error.
...
\subsection{Use projective integration in time}
Based upon \verb|homogenisationExample.m| of \cref{sec:HomogenisationExample}.
%\begin{figure}
...
time domain: macroscale steps need to resolve the macroscale oscillations of \cref{fig:homoLanLif1DSpec} with frequencies up to \(7.4(N-1)^2\) so set to the reciprocal; the burst
time needs to complete the decay of the fast sub-patch modes, rate~\(\alpha\cdot10^6\), so set the burst time to thrice the reciprocal.
However, need a bigger gap between the oscillations of the macroscale and the decay of the microscale, so increase damping parameter~\(\alpha\).
\begin{matlab}
%}
alpha = 1
dT = 0.1/(7.4*(nPatch-1)^2)
ts = 0:dT:0.01;
bT = 9e-6/alpha
addpath('../ProjInt')
[Mus,tss,Mss] = PIRK2(@heteroBurst, ts, M0(:), bT);
return
%{
\end{matlab}
