...
%{
\end{matlab}
\paragraph{INSERT some alternative interpolation}
Consider the case of Dirichlet BCs placed at the second point in the extreme patches.  We try a weighted sum of two linear approximations to the edge-value of the extreme patches.  Then are able to find the weights that restore symmetry in the diffusion (for random periodic diffusivities).
\begin{matlab}
%}
global bcwt
%  sizeF=size(F), sizeX=size(X), sizeUedge=size(Uedge)
%  F1=squeeze(F(:,:,:,:,1)), Xs=squeeze(X), Us=squeeze(Uedge)
wt=(nx-4)/(2*nx-6)+bcwt; % this for exploration
% following two lines is the exact weight!!
%dx=diff(patches.x(1:2)); m=nx-2; 
%wt=(m-2)/(2*m-2)+m*dx/(2-(m-1)*dx/3);
  dFdx=(F(2,:,:,2,1)-F(1,:,:,1,1))/(X(2,:,:,2)-X(1,:,:,1));
  Uedge(2,:,:,1)=wt*Uedge(2,:,:,1) ...
      +(1-wt)*(F(1,:,:,1,1)+diff(patches.x(1:2))*dFdx);
  dFdx=(F(1,:,:,end-1,1)-F(2,:,:,end,1))/(X(1,:,:,end-1)-X(2,:,:,end));
  Uedge(1,:,:,end)=wt*Uedge(1,:,:,end) ...
      +(1-wt)*(F(2,:,:,end,1)-diff(patches.x(1:2))*dFdx);
end%if
%{
\end{matlab}
