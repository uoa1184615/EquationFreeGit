...
\begin{figure}
\centering \caption{\label{fig:hyperDiffHeteroU}the
hyper-diffusing field~\(u(x,t)\) in the patch (gap-tooth)
scheme applied to microscale heterogeneous hyper-diffusion
(\cref{sec:hyperDiffHetero}).  The log-time axis shows:
\(t<10^{-2}\), rapid decay of sub-patch micro-structure;
\(10^{-2}<t<1\), a quasi-equilibrium; and \(1<t<10^2\), slow
decay of macroscale structures.}
...
coefficients. We discretise the \pde\ to a lattice of
values~\(u_i(t)\), with lattice spacing~\(dx\), and governed
by
\begin{equation*}
